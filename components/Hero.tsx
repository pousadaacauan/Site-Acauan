  ...
  return (
    <section className="relative w-full h-screen min-h-[750px] overflow-hidden bg-[#FDFBF7]">
      {HERO_IMAGES.map((img, index) => (
        ...
        >
          <img src={img} alt="Hero" className="w-full h-full object-cover brightness-[0.8] contrast-[1.1]" />
        </div>
      ))}
      ...
      <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-6">
        <div className="animate-fade-in-up max-w-5xl">
          ...
          <div className="flex flex-col sm:flex-row gap-8 justify-center">
            <a href="#products" onClick={(e) => handleNavClick(e, 'products')} className="px-14 py-5 bg-white text-[#7895B2] text-xs font-bold uppercase tracking-[0.3em] hover:bg-[#7895B2] hover:text-white transition-all duration-700 shadow-2xl">
              {t.btnRooms}
            </a>
            <a href="#about" onClick={(e) => handleNavClick(e, 'about')} className="px-14 py-5 border-2 border-white text-white text-xs font-bold uppercase tracking-[0.3em] hover:bg-white/10 transition-all backdrop-blur-md">
              {t.btnAbout}
            </a>
          </div>
        </div>
